...
{
    if(NiM_readParam(nAddr, 0x230,(uint16_t *) pOnline,2) == 0)//判断能否读取电机数据
    {
  ...
  uint16_t crc16 = 0;//16位CRC校验码
   ...
   buffer[0] = nAddr;
    ...
    delay_ms(1);//延时等待发送完成
		...
		timer2_cnt = 0;//定时器计数清零
    while(timer2_cnt < 500)//2s超时
    {
			/* BEGIN:   PN:3 */	
			if(RTU_FLAG == 1)//收到一帧完整数据
			{
					if(RS485_RX_BUF[0] == nAddr)//判断接收到的数据是不是正确的电机发出的
					{
							...
							else
							{
									...
									if((RS485_RX_BUF[1] == buffer[1]) && (RS485_RX_BUF[2] == len))//在电机非故障状态下，判断接收到的数据是否有问题
								{
									...
									RS485_RX_CNT=0;//接收重新计数
									timer2_cnt = 0;//定时器计数清零
									Serial_Printf("motor ok and done\r\n");
									break;
								}
							}
					}
				...
				RTU_FLAG=0;//清除接收完成标志位
					Serial_Printf("data trans finished\r\n");
			}
    }
		memset(RS485_RX_BUF, 0, 64);//清除接收缓存区的数据
		if(timer2_cnt >=2000)//判断是否接收超时
		{
			timer2_cnt=0;//定时器计数清零
...
{
	uint8_t error;
	error = NiM_writeParam(nCurAddr,0x230,(uint8_t *)&nNewAddr,2);
	if (error !=0)
		return error;
	return 0;
}
...
{
	uint8_t error;
	error = NiM_writeParam(nAddr,0x3C2,(uint8_t *)&nMode,1);
	if (error !=0)
		return error;
	return 0;
}
...
uint8_t NiM_moveVelocity(uint8_t nAddr, int32_t nVelocity)
{
	...
	if(nVelocity>0)
	{
		...
		error = NiM_writeParam(nAddr,0x3F3,(uint8_t *)(&temp2),2);
		if(error ==0)
		{error = NiM_writeParam(nAddr,0x448,(uint8_t *)(&nVelocity),4);
		}
	}
	else
	{
